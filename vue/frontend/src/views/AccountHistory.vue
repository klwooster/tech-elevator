<template>
  <div id="main">
      <applicant-filters v-bind:currentfilter="filters" header="Applications for Account ID 1" />
...
export default {
  ...
  methods: {
      getApplicants() {
          APIService.listApplicants()
            .then(applicantData => this.applicantList = applicantData.filter( (applicant) => {
                return applicant.accountId == 1;
            } ));
      },
      sort(evt) {
