...
public class JDBCHistoryDAO implements IHistoryDAO {
	...
	public List<History> getHistoryByChangedId(int id) {
		...
		String sqlGetHistoryByChangedId = "SELECT * FROM history WHERE history_id = ?";
	...
	public int logHistoryRecord(History history) {
		...
		jdbcTemplate.update(sqlSaveHistory, history.getDateOfChange(), history.getUpdateMadeById(), history.getChangesMadeToId(), history.getStatus());
